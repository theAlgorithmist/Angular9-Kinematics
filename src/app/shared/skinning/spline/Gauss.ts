...
/**
 * Typescript Math Toolkit: Gaussian quadrature.  Numerical integration of a function inside a specified interval, [a,b].
 ...
 * @version 1.0
 */
 export interface IEvalFcn
 {
   (t: number): number;
 }

 export class TSMT$Gauss
 {
   ...
   *
   * @param {IEvalFcn} _f Function to be integrated - must accept a numerical argument and return the function value at that argument.
   *
   ...
   *
   */
   public eval(_f: IEvalFcn, _a: number, _b: number, _n: number): number
   {
     let n: number = _n < 2 ? 2 : _n;
     n             = n > 8 ? 8 : n;

     let l: number   = (n==2) ? 0 : Math.floor( n*(n-1)/2 - 1 );
     let sum: number = 0;
     let i: number   = 0;
     ...
     {
       // change of variable
       let mult: number = 0.5*(_b-_a);
       let ab2: number  = 0.5*(_a+_b);
