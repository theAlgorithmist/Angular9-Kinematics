...
export class TSMT$CubicBezier extends PlanarCurve
{
  ...
  /**
   * Compute the normalized arc length at the given natural parameter value
   *
   * @param {number} t Natural parameter in [0,1]
   *
   * @returns {number}
   */
  public lengthAt(_t: number): number
  {
    if (isNaN(_t)) {
      return 0;
    }
    ...
    return this._integral.eval((t: number) => this.__integrand(t), 0, t, 8)
  }
